...
int SquareRoot(int x) {
  double upper = x, lower = 0, square = 0,
         mid = 0; // we use double for all the variables since we're dealing
                  // with floating point numbers.
  while (true) {
    mid = (upper + lower) / 2;
    square = mid * mid;
    if (square >= x &&
        square < x + 0.99) // we do the search until the square of the mid is
